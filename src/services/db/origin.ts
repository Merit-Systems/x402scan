...
export const getMostPopularOriginsAllTime = async (limit = 6) => {
  const origins = await listOriginsWithResourcesInternal({
    resources: {
      some: {
        response: {
...
export const getMostPopularOriginsThisWeek = async (limit = 6) => {
...
export const getNewestOrigins = async (limit = 6) => {
  const origins = await prisma.resourceOrigin.findMany({
    where: {
      resources: {
        some: {
