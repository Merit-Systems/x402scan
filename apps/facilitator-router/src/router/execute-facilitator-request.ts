...
import { Facilitator, FacilitatorMethod } from './types';
import { ContextHandler } from '../utils/context-handler';
import { logFacilitatorFailure, logFacilitatorSuccess } from './log-facilitator';

/**
 * Execute a single facilitator request
 */
export function executeFacilitatorRequest<T extends VerifyResponse | SettleResponse>(
        ...
        const result = await (async () => {
          switch (method) {
            case FacilitatorMethod.VERIFY:
              return await facilitatorClient.verify(payload, paymentRequirements) as T;
            case FacilitatorMethod.SETTLE:
              return await facilitatorClient.settle(payload, paymentRequirements) as T;
            default:
              throw new InvalidDataError(`Unknown method: ${method}`);
          }
        })();
