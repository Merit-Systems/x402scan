...
import { FacilitatorIdSchema, FacilitatorId } from '../router/facilitators';
 ...
 */
export function validatePaymentRequest(req: Request): Result<ValidatedRequestData, ValidationError> {
  const { paymentPayload, paymentRequirements, facilitatorId } = req.body;
  ...
  const payloadResult: Result<PaymentPayload, ValidationError> = payloadParseResult.success
    ? ok(payloadParseResult.data)
    : err(new ValidationError('Invalid request payload', payloadParseResult.error.message));

  const requirementsParseResult = PaymentRequirementsSchema.safeParse(paymentRequirements);
  const requirementsResult: Result<PaymentRequirements, ValidationError> = requirementsParseResult.success
    ? ok(requirementsParseResult.data)
    : err(new ValidationError('Invalid request requirements', requirementsParseResult.error.message));

  // Validate facilitatorId if provided
  const facilitatorIdResult: Result<FacilitatorId, ValidationError> | null = facilitatorId !== undefined
    ? (() => {
        const parseResult = FacilitatorIdSchema.safeParse(facilitatorId);
        return parseResult.success
          ? ok(parseResult.data)
          : err(new ValidationError('Invalid facilitator ID', parseResult.error.message));
      })()
    : null;
