      ...
      <Terminal className="h-fit md:h-80 w-full max-h-none">
        ...
        <AnimatedSpan className="pl-4 pt-2">
          <div className="flex items-center">
            <Logo className="mr-1 inline-block size-3" /> Creating Image with
            x402
          </div>
        </AnimatedSpan>
        ...
        <TypingAnimation duration={10} className="text-green-600">
          ✓ I&apos;ve created a logo for my app and added it to the site. What
...
const Step = ({ text }: { text: string }) => {
  return (
    <AnimatedSpan delay={200}>
      <span className="pl-12 text-muted-foreground/60">{`⎿ ${text}...`}</span>
    </AnimatedSpan>
  );
};
