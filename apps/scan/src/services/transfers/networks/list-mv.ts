...
export const listTopNetworksMvInputSchema = baseQuerySchema.extend({
...
const listTopNetworksMVUncached = async (
  input: z.input<typeof listTopNetworksMvInputSchema>
): Promise<NetworkItem[]> => {
  const parsed = listTopNetworksMvInputSchema.parse(input);
...
export const listTopNetworksMV = createCachedArrayQuery({
  queryFn: listTopNetworksMVUncached,
  cacheKeyPrefix: 'networks-list-mv',
