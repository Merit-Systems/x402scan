...
export const bucketedFacilitatorsStatisticsMvInputSchema = baseBucketedQuerySchema.extend({
  timeframe: z.nativeEnum(ActivityTimeframe).optional().default(ActivityTimeframe.OneDay),
});

const getBucketedFacilitatorsStatisticsMVUncached = async (
  input: z.infer<typeof bucketedFacilitatorsStatisticsMvInputSchema>
) => {
...
export const getBucketedFacilitatorsStatisticsMV = createCachedArrayQuery({
  queryFn: getBucketedFacilitatorsStatisticsMVUncached,
  cacheKeyPrefix: 'bucketed-facilitators-statistics-mv',
