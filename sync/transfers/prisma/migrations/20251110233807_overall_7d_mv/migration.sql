...
CREATE MATERIALIZED VIEW stats_aggregated_7d AS
SELECT 
...
WHERE t.block_timestamp >= NOW() - INTERVAL '7 days'
GROUP BY t.facilitator_id, t.chain;

CREATE UNIQUE INDEX stats_aggregated_7d_idx
ON stats_aggregated_7d (facilitator_id, chain);

CREATE MATERIALIZED VIEW stats_buckets_7d AS
SELECT
...
CREATE UNIQUE INDEX stats_buckets_7d_idx
ON stats_buckets_7d (bucket, facilitator_id, chain);