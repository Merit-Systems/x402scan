...
export const fsResultFromPromise =
  <Surface extends string>(surface: Surface) =>
  <T>(promise: Promise<T>, error: (e: unknown) => BaseFileSystemError) =>
    resultFromPromise<FileSystemErrorType, Surface, BaseFileSystemError>(
      surface
    )(promise, error);
...
export const fsErr =
  ...
  (error: BaseFileSystemError) =>
    err<FileSystemErrorType, Surface>(surface)(error);
